...
const { REACT_APP_WKND_ENDPOINT } = process.env;
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const response = await fetchPersistedQuery(
          REACT_APP_WKND_ENDPOINT + "/flightpackage-by-id",
          queryVariables
        );
...
export function useBAPageBySlug(slug, variation = "master", fetchTrigger) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await fetchPersistedQuery(
        REACT_APP_WKND_ENDPOINT + "/page-by-slug",
        queryVariables
      );
...
export function useChooseAFare(variation = "master", fetchTrigger) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        const response = await fetchPersistedQuery(
          REACT_APP_WKND_ENDPOINT + "/choose-a-fare",
          queryVariables
        );
